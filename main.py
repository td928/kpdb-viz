...
def main():
    ...
    phasing = st.sidebar.selectbox(
        "Select the phasing option to filter",
        options=("Within 5 Years", "From 5 to 10 Years", "After 10 Years"),
        index=0,
    )
    ...
    sources_select = st.sidebar.multiselect(
        "Select the sources you want to include in the map",
        options=sources,
        default=sources,
    )

    units_vs_projects = st.sidebar.radio(
        label="View of number of projects or units",
...
def visualize(tfed, phasing):
    ...
    fig = px.choropleth_mapbox(
        tfed,
        geojson=nta,
        color=phase_dict[phasing],
        locations=tfed["NTA2020"],
        featureidkey="properties.NTA2020",
        center={
            "lat": 40.5731022653906,
            "lon": -74.1052443755828,
        },  # -74.1052443755828, 40.5731022653906
